...
QGeoTileFetcherMapbox::QGeoTileFetcherMapbox(
    int scaleFactor, QGeoTiledMappingManagerEngine *parent)
    : QGeoTileFetcher(parent),
      m_networkManager(new QNetworkAccessManager(this)),
      m_userAgent(mapboxDefaultUserAgent), m_format("png"),
      m_replyFormat("png"), m_accessToken("") {
...
void QGeoTileFetcherMapbox::setFormat(const QString &format) {
  ...
  if (m_format == "png" || m_format == "png32" || m_format == "png64" ||
      m_format == "png128" || m_format == "png256")
    m_replyFormat = "png";
...
QGeoTileFetcherMapbox::getTileImage(const QGeoTileSpec &spec) {
  ...
  request.setUrl(
      QUrl(mapboxTilesApiPath +
           ((spec.mapId() >= m_mapIds.size()) ? QStringLiteral("mapbox.streets")
